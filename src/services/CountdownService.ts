...
export class CountdownService extends BaseApiService {
	...
	constructor() {
		...
		if (envTarget) {
			...
			if (isNaN(this.targetDate.getTime())) {
				console.warn(
					"Invalid TARGET_DATE environment variable. Falling back to default.",
				);
				this.targetDate = new Date("2026-07-01T00:00:00Z");
			}
		} else {
			console.warn(
				"TARGET_DATE environment variable not set. Falling back to default.",
			);
	...
	getCountdownStatus(now: number = Date.now()) {
		...
		return {
			...
			messages: {
				currentMessage: isReached
					? this.messages.sealBroken
					: this.messages.defaultMessage,
				currentSubtext: isReached
					? this.messages.somethingComing
					: this.messages.defaultSubtext,
